...
#SEMP
df_outliers$SEMP <- sign(df_numeric_no_cor$SEMP) * abs(df_numeric_no_cor$SEMP)^(1/4)
...
#PINCP
df_outliers$PINCP <- sign(df_outliers$PINCP) * abs(df_outliers$PINCP)^(1/4)

summary(df_outliers$PINCP)
sum(table(boxplot(df_outliers$PINCP, plot = FALSE)$out))

#INTP
df_outliers$INTP <- sign(df_outliers$INTP) * abs(df_outliers$INTP)^(1/4)
